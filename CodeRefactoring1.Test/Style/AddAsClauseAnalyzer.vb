    ...
    Public Class AddAsClauseAnalyzer
                ...
                Select Case context.Node.Kind
                    Case SyntaxKind.FunctionLambdaHeader, SyntaxKind.SubLambdaHeader
                        ...
                        For Each param As ParameterSyntax In _LambdaHeaderSyntax.ParameterList.Parameters
                            If param.AsClause Is Nothing Then
                                diag = Diagnostic.Create(Rule, param.GetLocation(), String.Format(MessageFormat, MessageFormat))
                    ...
                    Case SyntaxKind.VariableDeclarator
                        ...
                        If (Not node.MatchesKind(
                            ...
                            Debug.Print($"Node.IsKind list is missing {node.Kind.ToString}")
                        ...
                        If expressionConvertedType Is Nothing OrElse expressionConvertedType.ToString.ToLower.Contains("anonymous type:") Then
                            Exit Sub
                        End If

                        diag = Diagnostic.Create(Rule, VariableDeclaration.GetLocation(), String.Format(MessageFormat, MessageFormat))
                    Case Else
                        Stop
                End Select
                context.ReportDiagnostic(diag)
        ...
        Private Shared Sub AnalyzeClassForStatement(context As SyntaxNodeAnalysisContext)
            Try
                ...
                Dim diag As Diagnostic = Diagnostic.Create(Rule, ControlVariable.GetLocation(), String.Format(MessageFormat, MessageFormat))
                context.ReportDiagnostic(diag)
            Catch ex As Exception When ex.HResult <> (New OperationCanceledException).HResult
                Stop
                Throw
